...
trend <-  predict(lm(Run.Size ~ Year, data = dat))
dat$detrended <-  dat$Run.Size - trend



dat <- dat %>%
  summarize(Run=sum(detrended)) %>%
  mutate(mean.Run=mean(detrended)) %>%
  mutate(sd.Run=sd(detrended)) %>%
  mutate(std.Run=(Run-mean.Run)/sd.Run)
...
for (n in 1:nsims){
  ...
  cycle[n,] <- peak[n,] * sin(2 * pi * t / 4 + phi)             # 4‑period cycle
...
ER <- y * 0.01 + 0.15 # Mean of 0.15 with sd=0.01
