...
#Sgen = 784
  P_Sgen_NOS <- sapply(SMSE_list, function(x, Sgen = 784) {
    val <- rowSums(x@NOS[, 1, , y]) >= 784
    mean(val)
  })
#SMSY85 =1760
  P_Smsy85_NOS <- sapply(SMSE_list, function(x, SMSY = 1760) {
    val <- rowSums(x@NOS[, 1, , y]) >= 1760
    mean(val)
  })

  P_Sgen <- sapply(SMSE_list, function(x, Sgen = 7384) {
    val <- rowSums(x@NOS[, 1, , y] + x@HOS[, 1, , y]) >= 784
    mean(val)
  })

  P_Smsy85 <- sapply(SMSE_list, function(x, SMSY = 1760) {
    val <- rowSums(x@NOS[, 1, , y] + x@HOS[, 1, , y]) >=  1760
    mean(val)
  })
...
Ccol1000 <- c("0-250" = '#d01c8b', "251-500" = '#f1b6da',
             "501-750" = '#f7f7f7', "751-1000" = '#b8e186',
             ">1000" = '#4dac26')
...
Ccol1600 <- c("0-400" = '#d01c8b', "401-800" = '#f1b6da',
...
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_probPNI_", pop, file_suffix, ".png")),
...
if(scenario == "low"){
  g <- g %>%
    mutate(value = ifelse(value <= 250, "0-250",
                          ifelse(value > 250 & value <= 500, "251-500",
                                 ifelse(value > 500 & value <= 750, "501-750",
                                        ifelse(value > 750 & value <= 1000, "751-1000",
                                               ">1000")))))
  g$value <- factor(g$value, levels = c("0-250", "251-500", "501-750",
                                        "751-1000", ">1000"))
  col <- Ccol1000
}
...
gSp<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal,  fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_NatSp_", pop, file_suffix, ".png")),
...
gNSprob<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_NOSProb1500_", pop, file_suffix, ".png")),
...
gSgen<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal,  fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_Sgen_", pop, file_suffix, ".png")),
...
gSmsy85<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_Smsy_", pop, file_suffix, ".png")),
...
if(scenario == "low") {
  g <- g %>%
    mutate(value = ifelse(value < 400, "0-400",
                          ifelse(value >= 400 & value <= 800, "401-800",
                                 ifelse(value > 800 & value < 1200, "801-1200",
                                        ifelse(value >= 1200 & value < 1600, "1201-1600",
                                               ">1600")))))
  # Specify order of legend elements:
  g$value <- factor(g$value, levels = c("0-400", "401-800", "801-1200",
                                        "1201-1600", ">1600"))
  col <- Ccol1600
}
gRet<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_Ret_", pop, file_suffix, ".png")),
...
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_Rel_", pop, file_suffix, ".png")),
...
gc<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_catch_", pop, ".png")),
       gc, width = 5, height = 5)
}
...
ghoc<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal,  fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop,  paste0("icecream_HOcatch_", pop, file_suffix, ".png")),
...
gnoc<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal,  fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_NOcatch_", pop, file_suffix, ".png")),
...
gptc<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_ptcatch_", pop, file_suffix, ".png")),
...
gtc<- g %>% ggplot(aes(x = n_yearling, y = u_preterminal, fill = value, z = value)) +
  ...
  theme(legend.position = "top", legend.text = element_text(size = 10),
        legend.title = element_text(size = 13)) +
  guides(fill = guide_legend(ncol = 2), title.posigtion = "top") +
  labs(x = "Proportional change in\nhatchery releases", y = "Pre-terminal ER") +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme(panel.border = element_rect(colour = "black", fill = NA, linewidth = 1)) +
  ...
  coord_cartesian(clip = "off")
if(save.files){
  ggsave(file.path("figures", "SMSE", pop, paste0("icecream_tcatch_", pop, file_suffix, ".png")),
...
ggsave(file.path("figures", "SMSE", pop, paste0("gic1_", pop, file_suffix, ".png")), gic1,
       height = 9, width = 7)
