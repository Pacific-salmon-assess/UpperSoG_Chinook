...
Bio <- new(
  ...
  fec = fec * 0.95,# adjusting for pre-spawn survival from M. Clarke life-cycle table
...
# Take median AEQ UPT and UT in the last 6 years, and take the average of those values to project forward
UPTrange <-  UPT %>% apply(2, quantile, probs = c(0.025, 0.5, 0.975), na.rm=T) %>% t()
UPTmed <- UPTrange[seq((d$Ldyr-5),d$Ldyr), "50%"] %>% mean()
UTrange <-  UT %>% apply(2, quantile, probs = c(0.025, 0.5, 0.975), na.rm=T) %>% t()
UTmed <- UTrange[seq((d$Ldyr-5),d$Ldyr), "50%"] %>% mean()
